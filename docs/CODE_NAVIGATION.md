...
- Revision-bound diagnostic sessions: `crates/zryna-driver/src/diagnostic_sessions.rs` composes one retained `SourceMap` with unchanged diagnostic v2 bytes, bounded scheduling and final active-revision publication checks. Focus with `cargo test --locked -p zryna-driver diagnostic_sessions`; this is an internal host boundary, not a transport or semantic service.
