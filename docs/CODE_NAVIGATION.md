...
- Complete enum matching: `owned_aggregate_lowering/structured_match.rs` plans exhaustive sealed variants and active payloads; `structured_match_local.rs` commits joined locals. `M3_COMPLETE_ENUM_MATCHING_MATRIX.md` binds the #273 source, independent-IR and resource evidence while retaining #275 borrowing and all runtime/public exclusions.
