...
- Parser-side entry: `adapters/typescript-6/src/worker.mjs`, `worker-v3.mjs`, or `worker-v4.mjs`; choose the protocol explicitly. Native lexical entry: `crates/zryna-frontend/src/native_lexer.rs`; this is a source-bound partial stage only, before parsing, snapshot construction, provider handshake, or selection.
