...
handle authority; neither may add a competing count, clone, borrow, cleanup or runtime model.
Completion of one gap leaves the other blocking #270.
...
| --- | --- |
| Existing evidence versus gaps | `E1`–`E5` and `H1`–`H2` bind actual tests; only `G321` and `G322` are gaps, and both block #270 |
...
planning child only. Parent #270 remains blocked on both #321 and #322 plus its own final integrated
