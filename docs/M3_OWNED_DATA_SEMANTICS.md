...
- one exact final return of a supported acyclic Struct or FixedArray subobject from a canonical
  static field or constant array element under a parameter-free local root, materializing all
  descendants, masking the whole subtree, returning the unique same-type temporary, and excluding
  that returned owner from reverse survivor cleanup;
...
Vec, Shared, or Weak values, aggregate-subobject moves outside one exact direct local, one exact
parameter-free final return, or the exact
...
exact final return from a parameter-free local root, or move or clone into one exact same-type
static projection rooted in a distinct local. Nested enums,
...
| final-return static Struct/FixedArray subobject move | complete | one parameter-free local-root static projection, exact result type, complete source descendant topology, sole returned temporary, returned-owner cleanup exclusion, one-site sealing, and atomic value/place/transition/cleanup preflight |
...
Vec roots, supported String-bearing Struct, FixedArray, and root Enum values, mutable available
static StructField or FixedArrayConstant String leaves, and at most one private straight-line
aggregate projection site. That narrow aggregate site moves or explicitly clones a distinct fully
initialized exact same-type supported non-Copy Struct or FixedArray whole local into a mutable
available static projection. Move consumes the source; clone retains it, and exact prepare plus
initialized-prefix failure cleanup retains source and destination. Fresh/projected/partial sources
or other destinations, owned calls, CFG/public replacement, and second sites remain outside that
checkpoint. It resolves
...
dynamic or Vec projections, broader projected assignment or clone, calls, direct returns, CFG
transfer, or public owned functions.
...
For the final-return form of that static subobject move, with `M` missing canonical source-path
places, amplification is one value, one transition, and `M + D + 1` places, plus one return cleanup
plan and exactly `P` cleanup actions for the pending survivor roots. Checked addition and every value/place/transition/plan/action limit are
validated before descendant materialization, source masking, site-count mutation, or instruction
emission.

For a projected aggregate clone with `M` missing source-path places and `P` pending owners, exact
site amplification is one value, `M + 2` places for the path, temporary, and direct local, two
ownership transitions, two cleanup plans, and `2P + 1` cleanup actions. The producer checks every
counter before materializing the path or changing ownership. Complete descendant topology is not
emitted: the verified layout derives recursive clone capability, fallible String-leaf count, and
aggregate-prefix cleanup shape.

For the one projected aggregate assignment with `M` missing target-path places, exact site
amplification is one value, `M + 1` places for that canonical path plus the move-result temporary,
and two ownership transitions (`MoveFromPlace`, then `ReplacePlace`). It adds no cleanup plan or
cleanup action: replacement is the infallible commit, and verified IR derives the exact recursive
drop of the old target from its pre-state. Value, place, transition, path, source availability, and
site-count checks all finish before target-path materialization or ownership mutation.

For the static-subobject move form of that combined site, `S` is the number of missing canonical
source-path places, `D` the missing descendants below the selected source projection, and `T` the
missing target-path places. Exact amplification is one value, `S + D + T + 1` places for both
paths, the complete source subtree, and the sole move-result temporary, plus two ownership
transitions (`MoveFromPlace`, then `ReplacePlace`). It adds no cleanup plan, cleanup action, or
...
one value, `M + 1` places for the path plus clone-result temporary, two ownership transitions
(`ClonePlace`, then `ReplacePlace`), two cleanup plans, and `2P + 1` cleanup actions. Prepare failure
drops only the pre-existing roots, including source and destination; initialized-prefix failure
first drops the clone result prefix and then those roots. All counters are checked together before
path materialization or ownership mutation.
