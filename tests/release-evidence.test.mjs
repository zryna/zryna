...
function fixture(t, mutateStatement = () => {}, mutateSbom = (bytes) => bytes) {
      ...
      predicate: {
        buildDefinition: {
          ...
          externalParameters: { workflow: {
            path: '.github/workflows/release.yml', ref: 'refs/tags/v0.2.3',
            repository: 'https://github.com/zryna/zryna',
          } },
          ...
          resolvedDependencies: [{
            uri: 'git+https://github.com/zryna/zryna@refs/tags/v0.2.3',
            digest: { gitCommit: COMMIT },
          }],
        },
        runDetails: {
          builder: {
            id: 'https://github.com/zryna/zryna/.github/workflows/release.yml@refs/tags/v0.2.3',
          },
  ...
  return {
    ...
    environment: {
      ...
      GITHUB_REPOSITORY: 'zryna/zryna', GITHUB_REF: 'refs/tags/v0.2.3',
...
test('rejects provenance drift and cryptographic verifier failure', async (t) => {
  ...
  const provenance = join(drift.outputRoot,
    'zryna-0.2.3-x86_64-unknown-linux-gnu.intoto.jsonl');
...
test('signed-release admission rejects an envelope-bound header-only SBOM', async (t) => {
  const paths = fixture(t, () => {}, (_bytes, { target, archive }) => Buffer.from(`${
    canonicalBounded({
      ...
      name: `zryna-0.2.3-${target.triple}`,
      documentNamespace: `https://zryna.com/spdx/0.2.3/${target.triple}/${sha256(archive)}`,
    })}\n`));
  prepareAll(paths);
  await assert.rejects(() => verifySignedRelease({
    directory: paths.outputRoot,
  ...
  for (const [label, mutate] of [
    ...
    ['published URL', (asset) => {
      asset.browser_download_url = asset.browser_download_url.replace(
        'untagged-0123456789abcdefabcd', 'v0.2.3');
    }],
