...
int main(void) {
  ...
  string_bytes(&source, text, sizeof(text));
  fail_at = 0;
  assert(zryna_rt_o1_string_clone(&source, &result) == RT_OK);
  assert(result.pointer != source.pointer);
  string_bytes(&result, text, sizeof(text));
  assert(zryna_rt_o1_string_release(&result) == RT_OK);
  result = (zryna_rt_o1_handle){0};
  fail_at = allocation_attempt + 1;
