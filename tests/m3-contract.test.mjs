...
test("in-progress owned-data semantics separate the implemented checkpoint from closure", async () => {
  ...
  assert.match(document, /General structural Vec clone beyond String elements, owned aggregate clone, owned place projections\s+or assignment, general owned phi joins,\s+owned loop-carried phi joins, repeated or nested branches or loops, general lexical scope exits/);
  ...
  assert.match(document, /general structural Vec and aggregate clone, projections, assignment, and partial moves \| pending/);
  assert.match(architecture, /Vec construction, explicit clone for exact `Vec<bool>`, `Vec<i32>`, and `Vec<String>`/);
  assert.match(architecture, /General structural Vec clone beyond\s+String elements, owned aggregate clone/);
  assert.match(document, /controlled allocation\/capacity\/bounds\/UTF-8 fault closure \| in progress/);
  assert.match(document, /authenticated internal fault\/drop traces, including Vec<String> partial initialization, are complete.*executable target fault injection remains pending/);
