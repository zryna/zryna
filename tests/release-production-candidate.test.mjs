...
const FILES = {
  archive: `zryna-0.2.3-${TARGET}.tar.gz`,
  buildReceipt: `zryna-0.2.3-${TARGET}.build-receipt.json`,
  sbom: `zryna-0.2.3-${TARGET}.spdx.json`,
};
...
function authority() {
  ...
  return {
    ...
    intendedRelease: { ref: 'refs/tags/v0.2.3', tagProvenance: 'not-observed' },
...
function build(root, replica, archive = Buffer.from('candidate archive')) {
  ...
  const value = {
    ...
    productionAdmission: 'forbidden', version: '0.2.3', target: TARGET, replica,
...
test('candidate authority distinguishes observed main from an unobserved intended tag', () => {
  ...
  for (const mutate of [
    (copy) => { copy.source.ref = 'refs/tags/v0.2.3'; },
...
test('candidate replicas reproduce and installed acceptance remains publication-forbidden', async (t) => {
  ...
  const spawn = (executable, args, options) => {
    ...
    return { status: tampered ? 2 : 0, signal: null,
      stdout: Buffer.from(operation === '--version' ? 'zryna 0.2.3\n'
