...
function fixtures() {
  ...
  const input = {
    ...
    archive: {
      format: 'tar-gzip', root: `zryna-qualification-0.2.3-${TARGET}-${COMMIT.slice(0, 12)}`,
...
test('rejects production identity, unordered materials, and archive descriptor drift', async () => {
  ...
  production.input.source.ref = 'refs/tags/v0.2.3';
...
test('round-trips an authenticated Windows runtime executable through the real ZIP codec', async () => {
  ...
  input.archive = { format: 'zip',
    root: `zryna-qualification-0.2.3-${target}-${COMMIT.slice(0, 12)}`,
