...
test('package exposes the exact documented preflight entrypoint', async () => {
  ...
  assert.equal(
    ...
    'cargo test --locked -p zryna-backend-javascript && cargo test --locked -p zryna-backend-webassembly && cargo test --locked -p zryna-semantics --lib control_flow_v1 && cargo test --locked -p zryna-driver --lib module_closure && cargo test --locked -p zryna-driver --lib workspace_source::',
  );
});
