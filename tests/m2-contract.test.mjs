...
test('roadmap ledger exactly matches the digest-pinned issue graph and honest status', async () => {
  const [roadmap, architecture, frontends, status, moduleClosure] = await Promise.all(
    ['ROADMAP.md', 'ARCHITECTURE.md', 'FRONTENDS.md', 'STATUS.md', 'M2_MODULE_CLOSURE.md'].map((name) =>
  ...
  for (const row of rows) {
    ...
    assert.equal(row.state, [45, 46, 47, 48].includes(row.number) ? 'complete' : 'planned');
  }
  assert.match(
    roadmap,
    /Current status: contract specified, exact syntax protocol v3 implemented, deterministic module\s+closure implemented as an internal driver boundary, and the isolated IR verifier implemented, but\s+the M2 profile is not executable\./,
  );
  assert.match(roadmap, /digest-pinned planning inventory/);
  assert.match(architecture, /## Isolated `ControlFlowV1` boundary/);
  assert.match(architecture, /Neither internal boundary is connected to source semantics, any backend, or a[\s\S]*public command/);
  ...
  assert.match(status, /The public driver still selects protocol v2;[\s\S]*not connected to semantic lowering,[\s\S]*no backend accepts[\s\S]*no CLI command or manifest exposes it/);
