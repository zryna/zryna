//! Canonical fail-closed UTS repository architecture engine.
...
const CONTRACT_FILE: &str = "uts.workspace.json";
const CONTRACT_VERSION: u32 = 1;
const CONTRACT_PROFILE: &str = "uts-compiler-workspace-v1";
const MAX_CONTRACT_BYTES: u64 = 1024 * 1024;
const MAX_SCAN_ENTRIES: usize = 50_000;
const MAX_SCAN_DEPTH: usize = 32;
const IGNORED_DIRECTORY_NAMES: &[&str] = &[".git", ".uts", "dist", "node_modules", "target"];
...
/// Validates a complete UTS workspace and fails closed on incomplete inspection.
#[must_use]
...
fn canonical_workspace_root(root: &Path) -> Result<PathBuf, Diagnostic> {
    let metadata = fs::symlink_metadata(root).map_err(|error| {
        architecture_error(
            ...
            "select an existing regular directory containing uts.workspace.json",
        )
    })?;
...
fn load_contract(root: &Path) -> Result<WorkspaceContract, Diagnostic> {
    ...
    let metadata = fs::symlink_metadata(&path).map_err(|error| {
        architecture_error(
            ...
            "restore the canonical uts.workspace.json file",
        )
    })?;
    ...
    serde_json::from_str(&source).map_err(|error| {
        architecture_error(
            ...
            "match schemas/uts-workspace-v1.schema.json exactly; unknown fields are forbidden",
        )
    })
}
...
fn validate_contract_identity(contract: &WorkspaceContract, diagnostics: &mut Vec<Diagnostic>) {
    ...
    if contract.schema != "./schemas/uts-workspace-v1.schema.json" {
        diagnostics.push(architecture_error(
            "UTS-A1001",
            Some(Path::new(CONTRACT_FILE)),
            "workspace schema reference is not canonical",
            "set $schema to ./schemas/uts-workspace-v1.schema.json",
        ));
    }
    ...
    let expected_outputs = BTreeSet::from([".uts/cache", ".uts/out", "target"]);
    ...
    if actual_outputs != expected_outputs || contract.outputs.len() != expected_outputs.len() {
        diagnostics.push(architecture_error(
            ...
            "declare target, .uts/cache, and .uts/out exactly once",
        ));
    }
}
    ...
    for entry in entries {
        ...
        if !allowed.contains(name) {
            diagnostics.push(architecture_error(
                "UTS-A1004",
                Some(Path::new(name)),
                format!("root entry '{name}' is not part of the UTS architecture"),
        ...
        for entry in entries {
            ...
            if !allowed.contains(name.as_str()) {
                diagnostics.push(architecture_error(
                    ...
                    "move it into an allowed entry or update uts.workspace.json deliberately",
                ));
            }
        }
    }
    ...
    if let Some(manifest) = root_manifest {
        ...
        if cargo_members != contract_members {
            diagnostics.push(architecture_error(
                ...
                "Cargo workspace members differ from uts.workspace.json",
    ...
    for member in &contract.members {
        ...
        if actual_dependencies != expected_dependencies {
            diagnostics.push(architecture_error(
                ...
                "declare the exact same direct internal dependencies in Cargo.toml and uts.workspace.json",
            ));
        }
    }
}
...
fn validate_adapters(root: &Path, contract: &WorkspaceContract, diagnostics: &mut Vec<Diagnostic>) {
    for adapter in &contract.adapters {
        ...
        let expected_name = format!("@uts/adapter-{}", adapter.id);
            ...
            || metadata_worker != Some("src/worker.mjs")
        {
            diagnostics.push(architecture_error(
                "UTS-A1010",
                Some(&package_path),
                "adapter metadata differs from uts.workspace.json",
                "set uts.adapterId, uts.protocolVersion, and uts.worker to the registered values",
            ));
        }
        ...
        for dependency in &member.dependencies {
            ...
            if !allowed_layer_edge(member.kind, target.kind) {
                diagnostics.push(architecture_error(
                    ...
                    "depend only toward lower-level contracts or route orchestration through uts-driver",
                ));
            }
        }
    }
    ...
    *entries_seen += 1;
    ...
    if metadata.file_type().is_symlink() {
        diagnostics.push(architecture_error(
            "UTS-A1201",
            path.strip_prefix(root).ok(),
            "symlinks are forbidden inside controlled UTS components",
...
mod tests {
    ...
    fn rejects_unsafe_paths() {
        assert!(safe_relative_path("crates/uts-ir"));
    ...
    fn validates_canonical_ids() {
        assert!(valid_id("uts-backend-native"));
        assert!(!valid_id("UTS-native"));
