...
fn invalid_limits_reject_with_safe_correlation() {
    ...
    for (field, value) in [("work", 0), ("results", 0)] {
        ...
        let mut malformed: serde_json::Value =
            serde_json::from_slice(&request(&id, revision, 100_000, "diagnostics", json!({})))
        ...
        let response = session.begin_diagnostics(&bytes, now).expect_err("zero limit must reject");
...
fn invalid_correlation_fields_remain_uncorrelated() {
    let (mut session, revision, now) = ready_session("let x = 1;\n");
    for (field, value) in
        [("request_id", json!("")), ("snapshot", json!("non-ascii-é")), ("revision", json!(0))]
    {
        let mut malformed: serde_json::Value =
            serde_json::from_slice(&request("unsafe", revision, 100_000, "diagnostics", json!({})))
                .unwrap_or_else(|error| panic!("request must decode: {error}"));
        malformed[field] = value;
        let bytes = serde_json::to_vec(&malformed)
            .unwrap_or_else(|error| panic!("request must encode: {error}"));
        let response =
            session.begin_diagnostics(&bytes, now).expect_err("unsafe correlation must reject");
