...
pub(super) fn string_literal(
    ...
    with_record(program, result_ty, runtime, builder, |result, runtime, builder| {
...
pub(super) fn string_concat(
    ...
    with_record(program, ty, runtime, builder, |result, runtime, builder| {
        let left = place_storage_address(program, function, operation.places()[0], slots, builder)?;
        let right =
            place_storage_address(program, function, operation.places()[1], slots, builder)?;
...
pub(super) fn vec_construct(
    ...
    with_record(program, ty, runtime, builder, |result, runtime, builder| {
        ...
        for (index, value) in values.iter().copied().enumerate() {
            ...
            let address = builder
                .ins()
                .iadd_imm_u(data, i64::try_from(offset).map_err(|_| invariant_error())?);
...
pub(super) fn allocate_record(
    ...
    let layout = type_record(program, ty)?;
    allocate_with("zryna_m3_allocate_record", layout.size(), layout.alignment(), runtime, builder)
}
...
pub(super) fn allocate_bytes(
    ...
    allocate_with("zryna_rt_o1_allocate", size, alignment, runtime, builder)
}
...
fn allocate_with(
    ...
    call_ok(runtime, symbol, &[size, alignment, output_address], builder)?;
...
pub(super) fn call_ok(
    ...
    super::failure::runtime_status(runtime, status, builder);
    Ok(())
}
...
fn with_record(
    ...
    emit: impl FnOnce(
        ...
        &super::failure::Runtime<'_>,
        &mut FunctionBuilder<'_>,
    ) -> Result<(), Diagnostic>,
) -> Result<cranelift_codegen::ir::Value, Diagnostic> {
    let address = allocate_record(program, ty, runtime, builder)?;
