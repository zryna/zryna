...
fn named_import_mixed_argument_producers_are_ordered_and_later_failure_retains_earlier_owners() {
    ...
    let caller = program
        ...
        .next()
        .expect("fixture function");
    let block = caller.blocks().next().expect("fixture block");
    ...
    assert_eq!(
        ...
        [
            VerifiedCallArgument::Value(
                instructions[0].result().expect("result-producing fixture instruction")
            ),
            VerifiedCallArgument::Value(
                instructions[1].result().expect("result-producing fixture instruction")
            ),
            VerifiedCallArgument::Value(
                instructions[2].result().expect("result-producing fixture instruction")
            ),
        ]
    );
    assert_eq!(
