...
fn call_local(builder: &mut Builder) -> u32 {
    ...
    let id = u32::try_from(builder.statements.len()).expect("call statement");
...
fn callee(builder: &mut Builder, payload: &Ty, exclusive: bool) -> RawFunctionSyntax {
    let start = builder.source.len();
    ...
    let local_type = builder.ty(payload);
    builder.text(" ");
    let equals_span = builder.text("=");
    builder.text(" ");
...
fn fixture(exclusive: bool) -> (String, RawProjectSyntaxSnapshot) {
    let (mut builder, declarations, choice) = initial(&Element::Enum);
    ...
    builder.text("(): ");
    let result_type = builder.ty(&choice);
    builder.text(" ");
    let open_brace_span = builder.text("{");
    builder.text(" ");
    let root = builder.root_local(&choice);
    ...
    builder.text(" ");
    let alias =
        builder.borrow_alias(if exclusive { Case::Exclusive } else { Case::Shared }, &payload);
    let call = call_local(&mut builder);
    let inner_close = builder.text("}");
    builder.statements[nested as usize].span.end = inner_close.end;
    builder.text(" return ");
    let return_start = builder.source.len() - 7;
    let value = builder.reference("item");
    let semicolon_span = builder.text(";");
    let returned = u32::try_from(builder.statements.len()).expect("return");
    builder.statements.push(RawStatementSyntax {
        span: at(return_start, builder.source.len()),
        kind: RawStatementKind::Return {
            keyword_span: at(return_start, return_start + 6),
    ...
    builder.text(" ");
    let relay_function = callee(&mut builder, &payload, exclusive);
    (
        builder.source,
...
#[test]
fn refined_payload_borrows_pass_shared_and_exclusive_authority_to_direct_calls() {
    for exclusive in [false, true] {
        let (source, raw) = fixture(exclusive);
        let sources = sources_for(&source);
        let syntax = verify_snapshot(raw, &sources).expect("authenticated payload call");
        ...
        assert!(matches!(
            caller
                .places()
                .find(|place| place.id()
                    == instructions[begin].place_operands().next().expect("payload"))
                .expect("payload place")
                .kind(),
            VerifiedPlaceKind::EnumPayload { variant: 1, .. }
        ));
