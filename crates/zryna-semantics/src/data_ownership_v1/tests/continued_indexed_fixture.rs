...
impl Builder {
    pub(super) fn continued_container(&mut self, vector: bool, owned: bool, depth: usize) -> u32 {
        if depth == 0 {
    ...
    pub(super) fn continued_index_call(&mut self) -> u32 {
        let start = self.text.len();
    ...
    pub(super) fn continued_callee(&mut self) -> RawFunctionSyntax {
        self.text("\n");
        let start = self.text.len();
